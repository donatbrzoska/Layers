...
bool column_is_full(ColumnInfo c, int3 pg_size)
{
...
void paint_grid_copy(
    RWStructuredBuffer<ColumnInfo> src_pg_info, RWStructuredBuffer<Paint> src_pg_content, int3 src_pg_size, int2 src_pos,
    RWStructuredBuffer<ColumnInfo> dst_pg_info, RWStructuredBuffer<Paint> dst_pg_content, int3 dst_pg_size, int2 dst_pos)
{
    dst_pg_info[XY(dst_pos.x, dst_pos.y, dst_pg_size.x)] = src_pg_info[XY(src_pos.x, src_pos.y, src_pg_size.x)];

    for (int z = 0; z < src_pg_info[XY(src_pos.x, src_pos.y, src_pg_size.x)].size; z++)
    {
...
void paint_grid_fill(
    RWStructuredBuffer<ColumnInfo> pg_info, RWStructuredBuffer<Paint> pg_content, int3 pg_size,
    float pg_cell_volume, int pg_diffuse_depth, float pg_diffuse_ratio,
    int2 pos, Paint p)
{
    int initial_write_index = pg_info[XY(pos.x, pos.y, pg_size.x)].write_index;

    // fill paint into column
    Paint left = p;
    while (!is_empty(left) && !column_is_full(pg_info[XY(pos.x, pos.y, pg_size.x)], pg_size))
    {
        int z = pg_info[XY(pos.x, pos.y, pg_size.x)].write_index;
...
void paint_grid_reverse_transfer(
    RWStructuredBuffer<ColumnInfo> src_pg_info, RWStructuredBuffer<Paint> src_pg_content, int3 src_pg_size, int2 src_pos,
    RWStructuredBuffer<ColumnInfo> dst_pg_info, RWStructuredBuffer<Paint> dst_pg_content, int3 dst_pg_size, int2 dst_pos,
    float dst_pg_cell_volume, int dst_pg_diffuse_depth, float dst_pg_diffuse_ratio)
{
    for (int z = 0; z < src_pg_info[XY(src_pos.x, src_pos.y, src_pg_size.x)].size; z++)
    {
...
void paint_grid_delete(
    RWStructuredBuffer<ColumnInfo> pg_info, RWStructuredBuffer<Paint> pg_content, int3 pg_size,
    int3 delete_pos, float delete_volume)
{
...
Paint paint_grid_get(RWStructuredBuffer<ColumnInfo> pg_info, RWStructuredBuffer<Paint> pg_content, int3 pg_size, int3 get_pos)
{
